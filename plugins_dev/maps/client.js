(function () {
  ...
  window.BzlPluginHost.register("maps", (ctx) => {
    ...
    style.textContent = `
      ...
      .mapView { display:flex; gap: 12px; padding: 12px; min-height: 0; flex: 1; align-items: stretch; }
      ...
      .mapHud { width: 240px; border: 1px solid rgba(246,240,255,0.14); border-radius: 14px; background: rgba(255,255,255,0.02); padding: 10px; min-height: 0; height: 100%; max-height: 100%; overflow-y: auto; overflow-x: hidden; }
      ...
      /* Polygon editor: render inline in the same panel as the toggle (no full-screen modal). */
      .mapsPolyModal { display:block; margin-top: 12px; }
      .mapsPolyModalInner { width: 100%; overflow:hidden; border-radius: 18px; border: 1px solid rgba(246,240,255,0.14); background: linear-gradient(180deg, rgba(30,20,38,0.96), rgba(12,10,18,0.96)); box-shadow: 0 18px 60px rgba(0,0,0,0.35); padding: 14px; display:flex; flex-direction:column; }
      ...
      .mapsPolyGrid { margin-top: 12px; display:grid; grid-template-columns: 1fr; gap: 12px; min-height: 0; flex: 1; }
    ...
    function renderMapView() {
      ...
      if (canEditMap && editMode) {
        try {
          polyModalHtml = renderPolyModal();
        } catch (e) {
          devLog("error", "maps:renderPolyModal failed", { error: e?.message || String(e) });
            ...
            <div class="mapsPolyModal" id="mapsPolyModal">
              <div class="mapsPolyModalInner">
                <div class="mapsPolyHeader">
                  <div>
                    <div class="mapsPolyTitle">Polygon editor (fallback)</div>
                    <div class="small muted">UI render failed. You can still Close polygon and Save.</div>
                  </div>
                </div>
                <div class="small muted" style="margin-top:10px;">${msg}</div>
      ...
      const settingsHtml = showSettings
        ? `
          ...
          <div class="row" style="margin-top:10px; gap:10px;">
            <button type="button" class="${editMode ? "primary" : "ghost"} smallBtn" id="mapsEditToggle">Polygon editor tools${editMode ? " (ON)" : ""}</button>
      ...
      if (editMode !== lastEditModeLogged) {
        ...
        devLog("info", "maps:editMode", { editMode, canEditMap, mapId: activeMap?.id || "" });
      }
      if (editMode && canEditMap) {
        const now = Date.now();
        if (now - lastPolyUiLogAt > 1500) {
          lastPolyUiLogAt = now;
          const modal = document.getElementById("mapsPolyModal");
          const inHud = Boolean(modal && modal.closest(".mapHud"));
          const h = modal ? Number(modal.getBoundingClientRect().height || 0) : 0;
          devLog("debug", "maps:polyUi", { exists: Boolean(modal), inHud, height: h, draftPts: draftPoly.length, kind: editKind, tool: editTool });
        }
      }
      ...
      if (editBtn && canEditMap) {
        editBtn.onclick = () => {
          devLog("info", "maps:editToggle click", { before: editMode, mapId: activeMap?.id || "" });
          editMode = !editMode;
          draftPoly = [];
          polyDrag = null;
          vertexDrag = null;
          ...
          if (editMode) {
            ...
            devLog("info", "maps:editToggle off", { mapId: activeMap?.id || "" });
          }
          renderMapView();
        };
      }
    ...
    function commitDraftPoly() {
      ...
      if (editKind === "exit") {
        let name = String(exitDraftName || "").trim().slice(0, 40);
        if (!name) name = `Exit ${list.length + 1}`.slice(0, 40);
        const action = exitAction === "toMap" ? "toMap" : "toMaps";
        const selfId = String(activeMap?.id || "").trim().toLowerCase();
        const otherMaps = (Array.isArray(maps) ? maps : [])
          .map((m) => String(m?.id || "").trim().toLowerCase())
          .filter(Boolean)
          .filter((id) => id !== selfId)
          .sort((a, b) => a.localeCompare(b));
        let toMapId = action === "toMap" ? String(exitTargetMapId || "").trim().toLowerCase() : "";
        if (action === "toMap" && (!toMapId || toMapId === selfId)) {
          toMapId = otherMaps[0] || "";
        }
    ...
    function syncPolyDraftUi() {
      const ptsEl = document.getElementById("mapsPolyDraftPts");
      if (ptsEl) ptsEl.textContent = String(draftPoly.length);
      const closeDraft = document.getElementById("mapsPolyCloseDraft");
      if (closeDraft) closeDraft.toggleAttribute("disabled", draftPoly.length < 3);
      const undoPt = document.getElementById("mapsPolyUndoPt");
      if (undoPt) undoPt.toggleAttribute("disabled", !draftPoly.length);
      const clearDraft = document.getElementById("mapsPolyClearDraft");
      if (clearDraft) clearDraft.toggleAttribute("disabled", !draftPoly.length);
    }
        ...
        <div class="mapsPolyModal" id="mapsPolyModal">
          <div class="mapsPolyModalInner">
            ...
            <div class="row" style="gap:10px; flex-wrap:wrap; margin-top:10px; align-items:center;">
              <div class="small muted">Draft: <b id="mapsPolyDraftPts">${escapeHtml(String(draftPoly.length))}</b> pts</div>
    ...
    function syncPolyModal(canEditMap) {
      const existing = document.getElementById("mapsPolyModal");
      if (!(mode === "map" && canEditMap && editMode && activeMap)) {
        if (existing) existing.remove();
        return;
      }
      const html = String(renderPolyModal() || "").trim();
      if (!html) return;
    ...
    function wirePolyModalHandlers() {
      ...
      modal.onclick = (e) => {
        ...
        if (k) {
          ...
          if (!kind) return;
          devLog("info", "maps:polyKind", { from: editKind, to: kind });
          editKind = kind;
          const list = polysForKind(activeMap, editKind);
        ...
        if (t) {
          ...
          if (!tool) return;
          devLog("info", "maps:polyTool", { from: editTool, to: tool });
          editTool = tool;
          renderMapView();
          return;
        }
      ...
      if (closeDraft) {
        closeDraft.onclick = () => {
          const before = { kind: editKind, draftPts: draftPoly.length, action: exitAction, toMapId: exitTargetMapId };
          const ok = commitDraftPoly();
          devLog("info", "maps:closeDraft", { ok, ...before, exits: Array.isArray(activeMap?.exits) ? activeMap.exits.length : 0 });
      ...
      if (exitBehaviorEl) {
        exitBehaviorEl.onchange = () => {
          ...
          if (behavior === "toMap") {
            const want = String(exitToMapEl?.value || "").trim().toLowerCase() || (otherMaps && otherMaps.length ? otherMaps[0] : "");
            if (want) {
              if (exitToMapEl && exitToMapEl.value !== want) exitToMapEl.value = want;
              applyExitModel({ toMapId: want });
            }
          }
          syncExitVis();
          renderMapView();
      ...
      if (exitToMapEl) {
        if (!exitToMapEl.value && otherMaps.length) {
          exitToMapEl.value = otherMaps[0];
          // If we're editing draft defaults, keep the draft model in sync with the UI.
    ...
    function checkExits(position = localPos) {
      ...
      devLog("info", "maps:exitTriggered", {
        mapId: activeMap?.id || "",
        action,
        toMapId: String(triggered.toMapId || ""),
        targetExit: String(triggered.targetExit || ""),
      });
      if (action === "toMap") {
        ...
        const selfId = String(activeMap?.id || "").trim().toLowerCase();
        if (!to) {
          devLog("warn", "maps:exitMissingTarget", { mapId: activeMap?.id || "", action: "toMap" });
          leaveMap();
          return;
        }
        if (to === selfId) {
          devLog("warn", "maps:exitSelfTarget", { mapId: activeMap?.id || "", toMapId: to, action: "toMap" });
          leaveMap();
          return;
        }
        transitionToMap(to, targetExit);
        return;
      }
      leaveMap();
    }
