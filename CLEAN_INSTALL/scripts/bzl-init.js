...
function parseEnvTemplate(template) {
  ...
  for (const line of lines) {
    ...
    if (m) entries.push({ type: "kv", key: m[1], raw: line });
    else entries.push({ type: "raw", raw: line });
  }
  return entries;
}
...
async function main() {
  if (nodeMajor() < 18) fail(`Node.js 18+ required. Detected: ${process.versions.node}`);
  if (!fs.existsSync(ENV_EXAMPLE)) fail("Missing .env.example (expected at repo root).");
  ...
  try {
    ...
    let ownerUsername = "";
    if (createOwner) ownerUsername = await question(rl, "Owner username (a-z 0-9 _ . -)", { defaultValue: "owner" });
